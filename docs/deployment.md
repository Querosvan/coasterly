...
- `COASTERLY_ADMIN_EMAILS`
  - Comma-separated allowlist of trusted Google account emails.
  - When a verified Google email matches this list at sign-in time, that user is forced to `super_admin`.
  - This is a server-controlled bootstrap mechanism, not a client-side role assignment path.
...
7. Add `DATABASE_URL`, `CORS_ORIGIN`, `HOST`, `PORT`, `NODE_ENV`, `WEB_BASE_URL`, `SESSION_COOKIE_SECRET`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_REDIRECT_URI`, and `COASTERLY_ADMIN_EMAILS`.
...
11. Add any trusted operator Google accounts to `COASTERLY_ADMIN_EMAILS` if they must be bootstrapped as `super_admin`.
